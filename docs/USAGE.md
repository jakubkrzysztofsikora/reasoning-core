...
supplies an explicit exit status; it uses a decision link only when the host
supplies an explicit parent ID, otherwise recording a session-level receipt. It
never reruns commands or invents missing status. Missing links remain explicit;
