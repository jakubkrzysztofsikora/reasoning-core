...
| `RC_NEURAL_CORROBORATED` | `1` | Controls whether uncorroborated neural regressions are demoted to advisory. **Default `1`: when the sidecar reports `regression_detected` but no `deterministic_signal` is present, the regression is demoted to `decision=warn` with `neural_signal_mode="advisory"` (pre_edit_guard.py:1430-1436, mcp_gate.py:134-137). Flag `0`: the neural regression retains its original verdict and can hard-block under `S2_FAIL_CLOSED=1`.** This implements the AGENTS.md "neural is advisory unless corroborated" rule: default `1` makes neural signals advisory-only; setting `0` allows them to participate in hard-blocking. |
