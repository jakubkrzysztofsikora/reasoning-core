...
| (3) `RC_NEURAL_CORROBORATED` (and three other env vars) are undocumented production surface that converts the neural signal into a hard block, contra AGENTS.md | Yes — verified by grep across README, CONFIGURATION.md, HOW_IT_WORKS.md: zero mentions | **Retracted (round-3 retest):** the claim that "the `RC_SCORING_V3`, `RC_DIFF_WINDOWING`, `RC_MAHAL_CORPUS_MIN` flags are documented in `docs/CONFIGURATION.md`" is false. `grep -nE "RC_SCORING_V3|RC_DIFF_WINDOWING|RC_MAHAL_CORPUS_MIN|RC_NEURAL_CORROBORATED" docs/CONFIGURATION.md` returns zero matches in this repo at HEAD. The flags exist as env-var reads in `src/s2_core.py`, `src/hooks/pre_edit_guard.py`, and `src/mcp_gate.py`, but they are NOT documented in `docs/CONFIGURATION.md`, `README.md`, `HOW_IT_WORKS.md`, or anywhere else in the docs tree. **The round-3 work referenced in the original response was never landed** — the response cited work that does not exist in the tree. `RC_NEURAL_CORROBORATED` is indeed a real flag (read in `src/hooks/pre_edit_guard.py:1393` and `src/mcp_gate.py:134`) but remains undocumented operator surface. Round-3 task: actually document these four flags in `docs/CONFIGURATION.md`. |
...
| Claim | Why rejected |
|---|---|
| "RC-MAHAL-04 honest calibration" — the LOO approach is "still not calibrated; pick calibrated FPR or useful detector" | **Partially retracted (round-3 retest):** the substantive content (at n=5 the LOO FPR is conservative, detector is nearly inert at small corpus sizes) is correct. **However, the claim that "the docstring on `loo_threshold_for_fpr` has been updated to say so" is false.** `grep -nE "approximately 0.05|inert" src/scoring_signals.py` shows the docstring at `src/scoring_signals.py:189` still reads "With n=5 the realized FPR is approximately 0.05 (versus the 0.20 an in-sample quantile produces) — matches the host review's Finding 3 critique." The honest phrasing the response claims was added is NOT in the tree. Round-3 task: actually edit the docstring to say "at small corpus sizes the detector is nearly inert until the corpus reaches meaningful size." |
...
| Pre-existing tests | **Retracted (round-3 retest):** the "994 pass + 4 skip + 2 fail-as-designed — all unchanged" claim was the count immediately after round-4. Round-4 commit `00fdc0d` wrote `"BAAI/bge-code-v1": "REVIEWER_PIN_REQUIRED"` into `_PINNED_REVISIONS`, which broke `tests/test_security_hardening.py::test_pinned_revisions_are_40char_hex`. Round-3 fix: replaced the placeholder with the real 40-char SHA `bd67852057c5d7ddcc7b8234d9d6c410117ed851` and pinned the three Mamba-3 candidates with their live model-card SHAs. After this round's commit: **1041 pass + 4 skip + 2 fail-as-designed** (the 2 fails are the pre-existing Mamba-3 refusal-gate `test_pre_reg_embedder_gate.py` tests which are expected to fail until Mamba-3 is loadable). The "all unchanged" phrasing was wrong. |
