...
def _embed_all(texts: List[str]):
    ...
    Reviewer-flagged twice for broken API. Confirmed `ssm_backbone.embed`
    takes a STRING (line 226), tokenization happens internally. Both
    code and prose paths simply pass the raw text. Truncation at 8KB
    per text bounds tokenizer cost.
    """
    from src import ssm_backbone  # type: ignore
    out = []
    for t in texts:
        out.append(ssm_backbone.embed(t[:8000]))
    return out
