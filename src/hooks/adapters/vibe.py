"""Mistral Vibe CLI stdin -> HookEnvelope adapter.

Verification (2026-05-08, vibe v2.9.4): Vibe ships a ``post-agent-turn``
hook lifecycle today. PreToolUse hooks are not yet shipped (issues open
upstream); we approximate the gate via the ``gate_edit`` MCP tool.

This adapter parses the post-agent-turn payload so the post-turn audit
can retroactively flag missed ``gate_edit`` calls.
"""
