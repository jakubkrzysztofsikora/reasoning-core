...
install_envrc() {
  if [[ -e .envrc ]]; then
    ...
    # Mark the file as NOT ours so direnv_allow() won't auto-approve it.
    # An untrusted pre-existing .envrc must be reviewed by the operator
    # before being trusted; install.sh never calls 'direnv allow' on it.
    unset ENVRC_GENERATED_BY_INSTALL
    return
  fi
  export ENVRC_GENERATED_BY_INSTALL=1
  cat > .envrc <<EOF
  ...
  cat > "$target" <<'EOF'
{
  ...
  "hooks": {
    ...
    "PostToolUse": [
      ...
      { "matcher": "Edit|Write|MultiEdit",
        "hooks": [
          { "type": "command", "command": "${RC_PYTHON:-${RC_REPO}/.venv/bin/python} ${RC_REPO}/src/hooks/post_batch_lang_audit.py", "timeout": 5000 },
          { "type": "command", "command": "${RC_PYTHON:-${RC_REPO}/.venv/bin/python} ${RC_REPO}/src/hooks/post_edit_check.py", "timeout": 15000 }
        ] }
    ],
    "SessionStart": [
      { "hooks": [
...
# ---------------------------------------------------------------------------
direnv_allow() {
  # RC-SEC-06: only auto-approve a .envrc that THIS invocation of
  # install.sh just generated. A pre-existing attacker-controlled .envrc
  # is left alone so the operator sees the direnv trust prompt on first
  # 'cd' and can review the contents.
  if [[ "${ENVRC_GENERATED_BY_INSTALL:-0}" != "1" ]]; then
    if [[ -f .envrc ]]; then
      ok ".envrc present but not auto-approved (review before trusting)"
    fi
    return
  fi
